...
namespace Lab09_Entity_Framework
{
    public partial class Form1 : Form
    {
        ...
        private List<FoodModel> GetFoodByCategoryType(CategoryType cateType)
        {
            ...
            return dbContext.Foods.Where(x => x.Category.Type == cateType).OrderBy(x => x.Name).Select(x => new FoodModel()
            {
                Id = x.Id,
                Name = x.Name,
                Unit = x.Unit,
        ...
        private void tvwCategory_NodeMouseDoubleClick(object sender, TreeNodeMouseClickEventArgs e)
        {
            ...
            var caterogy = e.Node.Tag as Category;
            var dialog = new UpdateCategoryForm(caterogy?.Id);
            if(dialog.ShowDialog(this) == DialogResult.OK)
            {
                ShowCategories();
            }    
